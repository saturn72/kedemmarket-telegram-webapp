export default {
    addButton: "Add",
    address: "Address",
    addressAdd: "Add Address",
    addressAlias: "Address Alias",
    addressEdit: "Edit Address",
    approve: "Approve",
    back: "Back",
    backToBillingInfo: "Back to Billing Address",
    backToShippingAddress: "Back to Shipping Address",
    backToStore: "Back to Store 🏬",
    beginChatMessage: "🙋‍♀️ Hello!\nI have a question...\n\n",
    beginSupportChatMessage: "🙋‍♀️ Hello!\nI have a question regard site login...\n\n",
    billingInfo: "Billing Details",
    browsingToOrders: "Browsing to Orders",
    cancel: "Cancel",
    calculatingCart: "🧮 Calculating Cart... 🛒",
    callSupport: "📞 Call support: +972-54-2204119",
    cart: "Cart 🛒",
    cartProductsChanged: "Cart products have changed 🛒",
    carts: "Carts 🛒",
    cartTotal: "Cart Total:",
    checkout: "Checkout",
    checkoutCart: "💸 Checkout",
    city: "City",
    close: "Close",
    company: "Company Name",
    confirm: "Confirm",
    country: "Country",
    currencySymbol: "$",
    date: "Date",
    defaultAddress: "Default Address",
    deleteAddress: "🗑 Delete \'##0##\' from addresses?",
    deleteFromCart: "🗑 Delete ##0## from cart?",
    email: "Email",
    exit: "Exit",
    failedToFindPage: "Failed to find page...",
    failedToLocateOrder: "Failed to locate order...📃",
    failedToPerformAction: "Failed to perform action...🤦‍♂️",
    firstName: "First Name",
    for: "for",
    fullName: "Full Name",
    goToCart: "go to Cart 🛒",
    homePage: "Home Page 🏠",
    isRequired: "is required",
    itemTotal: "item's total",
    kedemmarket: "Kedem Market",
    lastName: "Last Name",
    loading: "loading...",
    loadingOrder: "loading order 🔃",
    loginIsRequiredToContinue: "Login is required to continue 🔓",
    loginIsRequiredToContinueWhy: "🔐In order to protect your personal information, login is required",
    logout: "Logout",
    missingBillingInfo: "Missing billing information",
    myOpenOrders: "My Open Orders",
    myOrders: "My Orders",
    next: "Next",
    noOpenOrders: "No Open Orders",
    noOrders: "No Orders",
    ooops: "Ooops 🤔",
    of: "of",
    orderId: "Order ID",
    orderItems: "Items",
    orderLink: "link to the order",
    orderNumber: "Order #",
    orderPlaced: "Order Placed",
    orderStatus: "Order Status",
    orderTotal: "Order Total",
    perUnit: "per unit",
    phoneNumber: "Phone Number",
    placingOrder: "Placing Order...",
    productSearch: "🔍 Search...",
    profile: "Profile",
    page: "Page",
    previous: "Previous",
    price: "price",
    pricesAfterDiscounts: "(Prices are after discount)",
    priceUpdatesAtCheckout: "📝 Price Updates at Checkout",
    productCartQuantity: "##0## product units in cart 🛒",
    requiredField: "value is required",
    repurchaseItem: "Repurchase Item",
    repurchaseOrder: "Repurchase Order",
    retry: "Retry",
    save: "Save",
    sendToWhatsapp: "Send to Whatsapp",
    shippingAddress: "Shipping Address",
    shippingAddresses: "Shipping Addresses",
    thankYouMessage: "🙏 Thank you for placing it!",
    toAccount: "Account",
    toCart: "Shopping Cart",
    toStore: "Store",
    total: "Total",
    unit: "units",
    unitPrice: "Unit Price'",
    update: "Update",
    updateBillingInfoIsRequired: "In order to purchase, billing details must be set",
    useBillingAddress: "Use Billing Address",
    usingBillingAddress: "Using Billing Address",
    youAreRedirectedToTheStore: "🏎 You'll be redirected to the store soon",
    yourCartIsEmpty: "Your Cart is Empty 🛒",
    zipPostalCode: "zipPostalCode",
}